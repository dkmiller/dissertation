...
increasing. Then there exists a sequence $\{x_n\}$ such that 
$\disc(\{x_n\}_{n\leqslant N},\mu) \ll \log(N) N^{-1}$
\end{theorem}
\begin{proof}
...
us that for $\{y_n\}$ the van der Corput sequence on $[0,1]$, we have 
\[
	\disc(\{\cdf_\mu^{-1}(y_n)\}_{n\leqslant N}, \mu) = \disc(\{y_n\}_{n\leqslant N})  ,
\]
which gives us the desired result with $x_n = \cdf_\mu^{-1}(y_n)$. 
\end{proof}

\begin{theorem}\label{thm:discrepancy-arbitrary}
Let $\mu$ be a measure on a closed interval such that the cdf is continuous, 
sends $0$ to $0$, and strictly increasing. Fix $\alpha\in (0,1)$. Then there 
exists a sequence $\{x_n\}$ such that 
$\disc(\{x_n\}_{n\leqslant N},\mu) = \Theta(N^{-\alpha})$. 
\end{theorem}
\begin{proof}
Without loss of generality, assume the closed integral in question is $[0,1]$. 
If $\{x_n\}_{n\leqslant N}$ is a sequence in $[0,1]$, let 
$\{x_n\}_{n\leqslant N+M}$ be the sequence $x_1,\dots,x_N,0,\dots,0$ ($M$ 
zeros). Then 
\[
	\disc^\star(\{y_n\}_{n\leqslant N+M},\mu)
		\geqslant \left| \frac{\#\{ n\leqslant N+M : x_n = 0\}}{N+M} - \mu\{0\}\right| 
		\geqslant \frac{M}{N+M} .
\]
On the other hand, 
\begin{align*}
...
which implies that 
\[
	\disc^\star\left(\{x_n\}_{n\leqslant N+M},\mu\right) \leqslant \disc^\star\left(\{x_n\}_{n\leqslant N},\mu\right) + \frac{2 M}{M+N} .
\]
Let $\{v_n\}$ be the $\mu$-equidistributed van der Corput sequence of 
Theorem \ref{thm:van-der-corput}. We know that 
$\disc(\{v_n\}_{n\leqslant N}, \mu) \ll N^{-\alpha}$. 

If discrepancy of $\{x_n\}_{n\leqslant N}$ is too small, add zeros to the end 
of the sequence, and the above argument shows the discrepancy will start to 
increase (but very slowly). If the discrepancy of $\{x_n\}_{n\leqslant N}$ is 
too big, continue adding van der Corput elements to the end. Note that the 
discrepancy is independent of ordering. So, if $\{x_n\}_{n\leqslant N}$ 
contains at most $M$ ``non-van der Corput elements'' then  note that (if we 
reorder things) for any $\epsilon>0$, 
$\#\{n\leqslant N : x_n\ne v_n\} = M$, which tells us that 
\[
	\disc^\star(\bx^N,\mu) \leqslant \disc^\star(\bv^N,\mu) + \frac{M}{N} \ll \frac{M+\log N}{N} .
\]
(This is the same as above. For $N$ sufficiently large and $M$ sufficiently 
small, we can make the discrepancy small again by adding sufficiently many 
van der Corput elements.)

Todo: polish this proof.
\end{proof}
...
$\{x_n\}$ is a sequence on $[a,b]$ and $\mu$ is a probability measure on 
$[a,b]$, then 
$\disc(\{x_n\}_{n\leqslant N}, \mu) = \disc(\{f(x_n)\}_{n\leqslant N}, f_\ast \mu)$, 
and likewise for star discrepancy. 
\end{lemma}
\begin{proof}
This is a simple computation, which we only check for star discrepancy: 
\begin{align*}
	\disc(\{f(x_n)\}_{n\leqslant N}, f_\ast \mu)
