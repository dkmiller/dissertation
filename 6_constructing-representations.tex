...
$\pi_{R_1}(x) \leqslant h(x) \pi(x)$ for all $x$, but there is a scalar 
multiple $h^\ast$ of $h$ so that $\pi_{R_1}(x) \leqslant h^\ast(x) \pi(x)$ for 
all $x$. 
...
$\sum \left(\frac{a_p}{2\sqrt p} - b_p\right)$ converges conditionally. Note 
that
\[
...
strictly increasing on that interval. For example, fix $\epsilon>0$, and let 
$\mu_\epsilon = B(t) \, \dd t$, where $B$ is a bump function on 
$\left[ \frac \pi 2 - \epsilon,\frac \pi 2 + \epsilon\right]$, normalized to 
have total mass one. Then Theorem \ref{thm:bad-Galois} gives Galois 
representations with empirical Sato--Tate distribution converging at any 
specified rate to $\mu_\epsilon$. This is a strictly stronger result than 
\cite[Th.~5.2]{pande-2011}. 
