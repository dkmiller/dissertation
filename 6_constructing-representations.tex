...
$\pi_{R\cup N}(x) \leqslant h(x)$ continues to hold. We also choose the 
...
$\frac{l^2}{2\sqrt p} > \min\left(2, \frac{l h(p)}{2\sqrt p}\right)$. 
...
$|U_k(\theta_{p_i}) + U_k(\theta_{q_i})| = |U_k(\theta_{p_i}) - U_k(- \theta_{q_i})|\ll p_i^{-1/2}$. 
We can then bound 
\[
