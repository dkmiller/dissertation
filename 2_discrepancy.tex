...
This gives a notion of discrepancy for sequences in $T$. Two different 
...
constant multiples of each other. So as far as asymptotics go, there is a 
well-defined discrepancy on $T$. Moreover, $\lambda$ is normalized Haar measure 
on the torus, we write $\D_N(\bvx)$ in place of $\D_N(\bvx, \lambda)$. 

Now let $G$ be a general compact, connected Lie group, and consider a sequence 
...
$\ft$ mapping to zero in $T$, it makes sense to talk about the discrepancy of 
a sequence in $G^\natural$ with respect to the Haar measure. Of course this 
definition depends on the choice of $Q$, but asymptotically these definitions 
are all the same. The paper 
...
easily be defined as a supremum over all open (or closed) balls---and in fact 
...
$\Omega$ with continuous joint distribution $\mu$. By this, we mean 
...
general versions for less restrictive notions of bounded variation. For 
example,  $f$ a function on $\bR^+$ that is bounded variation in the 
traditional sense (for example, piecewise continuous) and $\mu$ a continuous 
probability measure, the inequality 
\[
...
variation is much more complicated than the measure-theoretic one. See 
\cite[2\S5]{kuipers-niederreiter-1974} for a discussion. 
...
Throughout this section, $\lambda$ is the Lebesgue measure on $\bR$. 
...
\end{lemma}
\begin{proof}
Every isometry of $\bR$ is a composition of a translation and reflection. 
...
adding the same element to the end of a sequence eventually forces the 
discrepancy to increase, and that when doing this, the discrepancy does not 
increase (or fall) too rapidly. 
