...
Now let $E_{/\bQ}$ be a non-CM elliptic curve with Satake parameters 
...
we would have $\D_N(\btheta,\ST) \sim \pi(N)^{-\frac 1 2 + \epsilon}$. Since we 
...
$P_{\bx,N}$ is the empirical measure associated to $\{x_n\}_{n\leqslant N}$, 
so $P_{\bx,N} = \frac 1 N \sum_{n\leqslant N} \delta_{x_n}$. For all 
...
$\left|\D_N(\bx,\mu) - \D_N(f_\ast \bx,f_\ast\mu)\right| \leqslant \frac 2 N$.
\end{lemma}
\begin{proof}
Because $f$ is continuous and monotonic, given $[u,v)\subset I$, there exists 
$[x,y)\subset J$ such that $f[u,v)$ differs from $[x,y)$ by at most one 
element. Compute:
\[
	\left| P_{f_\ast\bx,N}[x,y) - f_\ast\mu[x,y) - \left(P_{\bx,N}[u,v) - \mu[u,v)\right)\right| \leqslant \frac 1 N ,
\]
the equality $f_\ast\mu[u,v) = \mu[x,y)$ following from the continuity of $\mu$. 
Since $[u,v)\subset I$ was arbitrary, it follows that 
$\D_N(\bx,\mu) \leqslant \D_N(f_\ast \bx,f_\ast\mu) + \frac 1 N$. 
...
\[
	\left| P_{\bx,N}[u,v) - \mu[u,v) - \left(P_{f_\ast \bx,N}[x,y) - f_\ast\mu[x,y)\right)\right| \leqslant \frac 2 N .
\]
Here, $[x,y)$ is arbitrary, so 
$\D_N(f_\ast \bx,f_\ast\mu) \leqslant \D_N(\bx,\mu) + \frac 2 N$. The result 
follows. 
\end{proof}
