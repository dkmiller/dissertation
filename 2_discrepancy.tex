...
of irreducible, nontrivial representations of the group. That entire 
discussion can be generalized to a much broader class of Dirichlet series, 
...
e.g.~$-1_{[0,\pi/2)} + 1_{(\pi/2,\pi]}$ on $[0,\pi]$. 
...
number $N\geqslant 1$, the empirical measure associated to the set 
...
If the measure $\mu$ is only defined on a Borel subset of $[0,\vinfty)^d$, we 
...
Diophantine approximation on tori, and the theory is most developed in that 
context. 
...
$\alpha\in \left(0,\frac 1 2\right)$. It can be useful to have a sequence 
...
dimensions. Its discrepancy has extremely fast convergence to zero. 
