...
lies in a real torus $T$, choose an isomorphism $T\simeq \bT^d = (\bR/\bZ)^d$, 
...
$G^\natural = T/W$, for $W$ the Weyl group of $T$. Choose an isomorphism 
$T \simeq \bT^d$, and as before we can identify $T$ as a measure space with 
...
definition depends on the choice of $T$, $Q$, and the isomorphism 
$T\simeq \bT^d$, but 
...
least $\D_N(\btheta,\ST) \ll N^{-\frac 1 2+\epsilon}$ (this is the 
