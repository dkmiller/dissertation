...
$\int f\, \dd\mu = 0$. One can restrict to any set of functions which 
...
where $\vx$ and $\vy$ range over $[0,\vinfty)$.
...
$[0,\vinfty)$, and $\mu$ a probability measure on $[0,\vinfty)$. For any 
...
$\D_N(\bx,\mu) = \D_N\left( P_{\bx,N},\mu\right)$, and similarly for star 
...
If the measure $\mu$ is only defined on a Borel subset of $[0,\vinfty)$, we 
...
$T$, it makes sense to talk about the discrepancy of a sequence in 
...
curve, the discrepancy $\D_N(\btheta,\ST)$ should decay like 
$\pi(N)^{-\frac 1 2}$, or at least $N^{-\frac 1 2+\epsilon}$. Similarly 
...
\begin{align*}
	\frac{1}{N} \sum_{n\leqslant N} f(\vx_n) - \int f\, \dd\mu 
		&= \frac{1}{N} \sum_{n\leqslant N} \left(f(\vx_n) - f(0)\right) - \int \left(f(\vx) - f(0)\right)\, \dd\mu(\vx) \\
		&= \frac{1}{N} \sum_{n\leqslant N} \int 1_{[0,\vx_n]}(\vy)\, \dd \nu(\vy) - \int \int 1_{[0,\vx]}(\vy)\, \dd\nu(\vy) \, \dd\mu(\vx) \\
...
apply this inequality. When $d>1$, the definition of variation is much more 
complicated. 
...
\end{lemma}
\begin{proof}
Every isometry of $\bR$ is a composition of translations and reflections. 
...
$P_{\bx,N}\{t\}\leqslant N^{-1}$. For any interval $[s,t)$ in $[0,\infty)$, we 
know that 
...
that $f^{-1}[x,y)$ differs from $[u,v)$ by at most two elements. In the first 
case, we compute:
\[
