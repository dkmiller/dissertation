...
$[0,\vinfty) = [0,\infty)^d$. For vectors $\vx,\vy\in [0,\infty)^d$, 
we say $\vx<\vy$ if $x_1<y_1$,\dots,$x_d<y_d$, and in that case write 
$[\vx,\vy)$ for the half-open box $[x_1,y_1)\times \cdots \times [x_d,y_d)$. 
...
$[0,\infty)^d$, we have 
\[
	\left| \frac{1}{N} \sum_{n\leqslant N} f(x_n) - \int f\, \dd\mu \right| \leqslant \Var(f) \D(\bx^N,\mu) .
\]
\end{theorem}
\begin{proof}
...
\]
The supremum in question is clearly bounded above by $\D(\bx^N,\mu)$, so the 
proof is complete. 
\end{proof}
...
\[
	\left| \frac{1}{N} \sum_{n\leqslant N} f(x_n) - \int f\, \dd\mu\right| \leqslant \Var(f) \D^\star(\bx^N,\mu) 
\]
...
\]
Moreover, we have 
$\left| \bx^N[0,t+\epsilon) - \nu[0,t+\epsilon)\right| \leqslant \D^\star(\bx^N,\nu)$. Putting these together, we get: 
\begin{align*}
	\by^N[0,t) - \nu[0,t) 
		&\leqslant \bx^N[0,t+\epsilon) - \nu[0,t) + \frac{\#\{ n\leqslant N : |x_n - y_n| \geqslant \epsilon\}}{N} \\
		&\leqslant \nu[t,t+\epsilon) + \D^\star(\bx^N,\nu) + \frac{\#\{ n\leqslant N : |x_n - y_n| \geqslant \epsilon\}}{N} \\
		&\leqslant \left\|\frac{\dd\mu}{\dd\lambda}\right\|_\infty \epsilon + \D^\star(\bx^N,\nu) + \frac{\#\{ n\leqslant N : |x_n - y_n| \geqslant \epsilon\}}{N} 
\end{align*}
This tells us that 
\[
	\D^\star(\by^N,\nu) \leqslant \left\|\frac{\dd\mu}{\dd\lambda}\right\|_\infty \epsilon + \D^\star(\bx^N,\nu) + \frac{\#\{ n\leqslant N : |x_n - y_n| \geqslant \epsilon\}}{N} .
\]
...
\[
	\left|\D(\bx^N, \nu) - \D(\sigma_\ast \bx^N, \sigma_\ast \nu)\right| \leqslant \frac{2}{\pi(N)} .
\]
\end{lemma}
\begin{proof}
...
Then 
\[
	\max\{\D(\bx^N,\mu),\D(\by^N,\nu)\} \leqslant \D((\bx\wr\by)^N, \mu+\nu) \leqslant \D(\bx^N,\mu) + \D(\by^N,\nu)
\]
\end{theorem}
\begin{proof}
...
\begin{align*}
	\left| (\bx\wr\by)^N([a,b)\sqcup [s,t)) - (\mu+\nu)([a,b)\sqcup[s,t))\right| 
		...
		&\leqslant \D(\bx^N,\mu) + \D(\by^N,\nu) .
\end{align*}
This yields the second inequality in the statement of the theorem. To see the 
first, assume that the maximum discrepancy is $\D(\bx^N,\mu)$, and let 
$[s,t)$ be a half-open box such that $|\bx^N[s,t) - \mu[s,t)|$ is within an 
arbitrary $\epsilon$ of $\D(\bx^N,\mu)$. We can assume that $[s,t)$ does not 
...
$\D(\bv^N) \leqslant \frac{\log(N+1)}{N\log 2}$. 
\end{lemma}
\begin{proof}
This is \cite[Ch.~2 Th.~3.5]{kuipers-niederreiter-1974}. In particular, we 
will use often that $\D(\bv^N)\ll \frac{\log N}{N}$. 
\end{proof}
...
$\bx=(x_1,x_2,\dots)$ such that $\D(\bx^N,\mu) \ll \frac{\log(N)}{N}$. 
\end{theorem}
\begin{proof}
...
on $[0,1]$, we have $\D(\cdf_\mu^{-1}(\bv)^N, \mu) = \D(\bv^N,\mu)$, 
...
\end{align*}
which implies that 
$\D^\star\left(\bx^N:a^M,\mu\right) \leqslant \D^\star\left(\bx^N,\mu\right) + \frac{2 M}{M+N}$. 
...
$\D^\star(\bx^{N_1},\mu) < N_1^{-\alpha}$. Then set $x_{N_1+1} = a$, 
$x_{N_1+2} = a$, \dots, until 
$\D^\star(\bx^{N_1+M_1},\mu) > (N_1+M_1)^{-\alpha}$. Then set 
$x_{N_1+M_1+1} = v_{N_1+1}$, $x_{N_1+M_1+2} = v_{N_1+2}$, \dots, 
until once again 
$\D^\star(\bx^{N_1+M_1+N_2},\mu) < (N_1+M_1+N_2)^{-\alpha}$. Repeat 
indefinitely. We will show first, that the two steps are possible, and that 
nowhere does $\D^\star(\bx^N,\mu)$ differ by too much from 
$N^{-\alpha}$. 
...
step. So if $\D^\star(\bx^N,\mu) < N^{-\alpha}$, we can ensure that 
$\D^\star(\bx^N:a^M,\mu)$ is at most $N^{-1}$ greater than $N^{-\alpha}$. 

Moreover, we know that $\D^\star(\bx^N:a,\mu)$ is at most 
$\frac{2}{N+1}$ away from $\D^\star(\bx^N,\mu)$. So when adding van der 
...
\[
	\left|\D^\star(\bx^N,\mu) - N^{-\alpha}\right| \ll N^{-1} , 
\]
