...
\begin{align*}
	\frac{1}{\dd(n \vx,\bZ^d)} 
		&\ll |n|^{\omega_0(\vx)+\epsilon}\qquad \text{ for $n\in \bZ\smallsetminus 0$} \\
	\text{(resp.~}
	\frac{1}{\dd\left(\langle \vm,\vx\rangle, \bZ\right)} 
		&\ll |\vm|_\infty^{\omega_{d-1}(\vx)+\epsilon} \qquad\text{ for $\vm\in \bZ^d\smallsetminus \vzero$).}
\end{align*}
\end{lemma}
\begin{proof}
...
$\bQ$, and $\omega_0(\vx)$, $\omega_{d-1}(\vx)$ are finite. Let 
...
$\langle \vm,\vx\rangle\in \bR\smallsetminus 0$. Thus Lemma 
...
Let $\vx\in \bT^d$ generate a dense subgroup.  The 
associated sequence of ``fake Satake parameters'' is 
$\bvx = (\vx, 2\vx, 3 \vx, \dots)$, where we put $\vx_{\fp_n} = n \vx$. For 
any fixed $w\geqslant \frac 1 d$, by Theorem \ref{thm:jarnik}, we can find 
$\vx$ with $\omega_0(\vx) = w$ and $\omega_{d-1}(\vx) = d w + d - 1$. 
