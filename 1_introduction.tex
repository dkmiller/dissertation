...
representation, then $L(1,s) = \zeta(s)$, the Riemann zeta function. Given a 
...
continuation to $\Re = 1$, for there is a simple pole at $s = 1$. So the 
result is only true when $K/\bQ$ is a nontrivial extension. Returning to that 
case ($K\ne \bQ$), there is a version of the strong prime number theorem in 
