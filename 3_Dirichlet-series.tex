...
are isogenous, so in particular they have the same rank. 

To-do: numerical experiments on recovering rank from $L_{\sgn}(E,1)$. 
...
the term $N^{-s} \sum_{p\leqslant N} x_p$ converges to zero, we get 
\[
...
\end{equation}
For $f\in L^1(\SU(2)^\natural)$, recall that the Fourier coefficients of $f$ 
are 
\[
	\widehat f(\sym^k) = \int_{\SU(2)^\natural} f(x) \tr\sym^k(x)\, \dd x .
\]
The Peter--Weyl theorem gives 
$f = \sum_{k\geqslant 1} \widehat f(\sym^k) \tr\sym^k$. 
Equation \eqref{eq:katz-char-bound} tells us that for any 
$f\in C(\SU(2)^\natural)$ with 
$\sum_{k\geqslant 1} |\widehat f(\sym^k)|<\infty$ and $\widehat f(\sym^1) = 0$, 
